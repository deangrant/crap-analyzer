//! Collect Go sources under an analysis root.
...
const SKIP_ALWAYS: &[&str] = &["vendor", ".git", "testdata"];
...
/// Walks `root` for `.go` files, skipping nested module roots in `skip`.
///
/// # Errors
///
/// Returns [`Error::Io`] if the root cannot be canonicalized or a directory
/// cannot be read.
pub fn go_files(root: &Path, skip: &[PathBuf]) -> Result<Vec<PathBuf>> {
    let mut out = Vec::new();
...
fn is_nested_module(path: &Path, walk_root: &Path) -> bool {
    path != walk_root && path.join("go.mod").is_file()
}
...
fn take_typed_entry(
    ...
    collect_go_file(path.to_path_buf(), walk);
    Ok(())
}
...
fn collect_go_file(path: PathBuf, walk: &mut Walk<'_>) {
...
fn skip_dir(dir: &Path, walk_root: &Path, nested_skip: &[PathBuf]) -> bool {
    ...
    if is_nested_module(dir, walk_root) {
        return true;
    }
    dir.file_name()
        .and_then(|n| n.to_str())
