//! Cognitive complexity for a TypeScript function body.
...
impl CognitiveCounter {
    ...
    fn take_token(&mut self, bytes: &[u8], i: usize) -> Option<usize> {
        ...
        for word in [b"if" as &[u8], b"for", b"while", b"do", b"switch", b"catch"] {
    ...
    fn scan_bools(&mut self, bytes: &[u8]) {
        ...
        while i < bytes.len() {
            ...
            i = advance_bool_scan(bytes, i, &mut self.count);
        }
    }
}

fn advance_bool_scan(bytes: &[u8], i: usize, count: &mut usize) -> usize {
    if let Some(end) = consume_bool_run(bytes, i) {
        *count += 1;
        return end;
    }
...
fn find_block_open(bytes: &[u8], mut i: usize) -> Option<usize> {
    while i < bytes.len() {
        i = skip_noise(bytes, i);
        match block_open_at(bytes, i) {
            BlockScan::Open(at) => return Some(at),
            BlockScan::Stop => return None,
            BlockScan::Continue => {}
        }
        i += 1;
    }
...
fn block_open_at(bytes: &[u8], i: usize) -> BlockScan {
    if i >= bytes.len() {
        return BlockScan::Stop;
    }
    if bytes[i] == b'{' {
        return BlockScan::Open(i);
    }
    if bytes[i] == b';' {
        return BlockScan::Stop;
    }
