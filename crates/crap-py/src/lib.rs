...
/// Python implementation of [`Language`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PyLanguage {
    /// Analyze every project under the workspace root.
    pub workspace: bool,
...
impl Language for PyLanguage {
    fn resolve_targets(&self, request: &ScanRequest) -> Result<Vec<Target>> {
        ...
        if let Some(targets) = Self::targets_from_project_root(&request.path)? {
            return Ok(targets);
        }
...
impl PyLanguage {
    ...
    fn targets_from_project_root(path: &Path) -> Result<Option<Vec<Target>>> {
...
fn collect_functions(targets: &[Target], metric: Metric) -> Result<Vec<LocatedFn>> {
    ...
    if failed > 0 {
        return Err(Error::collect(collect_failure(
            "Python", failed, succeeded, &details,
        )));
    }
    Ok(functions)
}
...
fn collect_target(
    ...
    let files = walk::py_files(&target.root, &target.skip)?;
    for file in files {
        if take_file(
            &file,
