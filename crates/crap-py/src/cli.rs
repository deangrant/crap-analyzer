//! Command-line flags and help text for `crap-py`.

// dry-rs:ignore-file. intentional parallel language frontend; keep separate.
use crate::PyLanguage;
use clap::Parser;
...
pub struct Args {
    ...
    /// Walk root. A project root (`pyproject.toml`) is analyzed per project.
    ...
    /// Analyze every project under the workspace.
    #[arg(long, conflicts_with = "packages")]
    pub(crate) workspace: bool,
    /// Selected project names (`-p`).
...
impl Args {
    /// Splits Python-only flags from the language-agnostic scan request.
    #[must_use]
    pub fn parts(&self) -> (PyLanguage, ScanRequest) {
        (
            PyLanguage {
...
pub fn help_text() -> String {
    format!(
        "\
{name} {version}
Score Python functions by combining complexity and test coverage.
...
OPTIONS:
    --coverage <file>       LCOV coverage file [default: lcov.info].
                            Produce one with coverage.py
                            (`coverage lcov -o lcov.info`). Paths in
                            the file must resolve to .py sources.
    --path <dir>            Walk root [default: .]. A project root
                            (pyproject.toml) is analyzed per project
    ...
    --workspace             Analyze every project under the workspace
    -p, --package <name>    Analyze only this project name (repeatable)
    ...
    --missing <policy>      No coverage data, empty span, or an
                            ...
                            A project name breaks equal basename ties.
...
mod tests {
    ...
    fn version_text_includes_crate_name() {
        ...
        assert!(text.starts_with("crap-py "));
        assert!(text.contains(env!("CARGO_PKG_VERSION")));
    }

    #[test]
    fn parts_split_py_flags_from_the_request() {
        let action = parse_args(argv(&[
            "--coverage",
            "c.info",
            "--workspace",
    ...
    #[test]
    fn help_text_mentions_lcov_and_python() {
        let text = help_text();
        assert!(text.contains("LCOV"));
        assert!(text.contains("Python"));
        assert!(text.contains("crap-py [OPTIONS]"));
    }
}
